...
class GlobScanner {
	...
	public GlobScanner(File rootDir, List<String> includes,
		...
		try {
			...
			throw new RuntimeException("OS error determining canonical path: "
					+ rootDir, ex);
		}
		...
		List<Pattern> allExcludePatterns = new ArrayList<Pattern>(
				excludes.size());
		...
		if (!allExcludePatterns.isEmpty()) {
			...
			outerLoop:
			//
			for (Iterator<String> matchIter = matches.iterator(); matchIter
					.hasNext();) {
				...
				try {
					...
					// name.
					for (Iterator<Pattern> excludeIter = excludePatterns
							.iterator(); excludeIter.hasNext();) {
						Pattern exclude = (Pattern) excludeIter.next();
						if (exclude.values.length == 2
								&& exclude.values[0].equals("**")) {
							exclude.incr();
	...
	public static void main(String[] args) {
		...
		List<String> files = new GlobScanner(new File(".."), includes, excludes)
				.matches();
