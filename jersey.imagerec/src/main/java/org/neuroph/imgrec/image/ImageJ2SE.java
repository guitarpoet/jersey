...
/**
 * This class represents image on J2SE platform, and the Image interface is used
 * to provide compatibility with Android platform. It is a wrapper around
 * BufferedImage and implementation of Image interface.
 * 
 * @author dmicic
 */
public class ImageJ2SE implements Image {
	...
	public ImageJ2SE(Integer width, Integer height, Integer imageType) {
		bufferedImage = new BufferedImage(width, height, checkImageType(imageType));
	}

	public ImageJ2SE(File imageFile) throws IOException {
		bufferedImage = ImageIO.read(imageFile);
	}

	public ImageJ2SE(String filePath) throws IOException {
		bufferedImage = ImageIO.read(new File(filePath));
	}

	public ImageJ2SE(URL imageUrl) throws IOException {
	...
	public Image crop(int x1, int y1, int x2, int y2) {
		...
		// ((ImageJ2SE) image).setBufferedImage(((ImageJ2SE)
		// image).getBufferedImage().getSubimage(x1, y1, x2 - x1, y2 - y1));
		// return image;
	}
