 ...
 * networks. Its one of the most efficent learning rules for this type of
 * networks, and it does not require setting of learning rule parameter.
 * 
...
public class ResilientPropagation extends BackPropagation {
	...
	private static final double ZERO_TOLERANCE = 1e-27; // the lowest limit when
														// something is
	...
	protected void onStart() {
		...
		// create ResilientWeightTrainingtData objects that will hold additional
		// data (resilient specific) during the training
	...
	/**
	 * Calculate and sum gradients for each neuron's weight, the actual weight
	...
	protected void updateNeuronWeights(Neuron neuron) {
		for (Connection connection : neuron.getInputConnections()) {
			...
			// ... and get the object that stores reislient training data for
			// that weight
			ResilientWeightTrainingtData weightData = (ResilientWeightTrainingtData) weight.getTrainingData();

			// calculate the weight gradient (and sum gradients since learning
	...
	protected void resillientWeightUpdate(Weight weight) {
		...
		if (gradientSignChange > 0) {
			...
			// weightChange = -sign(weightData.gradient) * delta; // if error is
			// increasing (gradient is positive) then subtract delta, if error
			// is decreasing (gradient negative) then add delta
			...
			// weightChange = - weightData.previousDelta;// 0;// -delta -
			// weightData.previousDelta; // ovo je problematicno treba da bude
			// weightChange
			weightChange = -weightData.previousWeightChange; // if it skipped
															 // min in previous
															 // step go back
		...
		weightData.previousGradient = weightData.gradient; // as in
														   // moveNowValuesToPreviousEpochValues
		weightData.gradient = 0;
	}
