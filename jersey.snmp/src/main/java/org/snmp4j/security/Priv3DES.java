...
public class Priv3DES implements PrivacyProtocol {
	...
	private static final long serialVersionUID = 2990170242137898581L;
	...
	public byte[] encrypt(byte[] unencryptedData, int offset, int length, byte[] encryptionKey, long engineBoots,
		...
		if (encryptionKey.length < 32) {
			...
			throw new IllegalArgumentException("encryptionKey has illegal length " + encryptionKey.length
	...
	public byte[] decrypt(byte[] cryptedData, int offset, int length, byte[] decryptionKey, long engineBoots,
	        long engineTime, DecryptParams decryptParams) {
		if ((length % 8 != 0) || (length < 8) || (decryptParams.length != 8)) {
			throw new IllegalArgumentException("Length (" + length + ") is not multiple of 8 or decrypt "
			        + "params has not length 8 (" + decryptParams.length + ").");
		}
		if (decryptionKey.length < 32) {
			logger.error("Wrong Key length: need at least 32 bytes, is " + decryptionKey.length + " bytes.");
			throw new IllegalArgumentException("decryptionKey has illegal length " + decryptionKey.length
			        + " (should be at least 32).");
		}
	...
	public byte[] extendShortKey(byte[] shortKey, OctetString password, byte[] engineID,
