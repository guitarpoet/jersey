...
public abstract class AuthGeneric implements AuthenticationProtocol {
	...
	public boolean authenticate(byte[] authenticationKey, byte[] message, int messageOffset, int messageLength,
	...
	public boolean isAuthentic(byte[] authenticationKey, byte[] message, int messageOffset, int messageLength,
	        ByteArrayWindow digest) {
		// copy digest from message
		ByteArrayWindow origDigest = new ByteArrayWindow(new byte[MESSAGE_AUTHENTICATION_CODE_LENGTH], 0,
		        MESSAGE_AUTHENTICATION_CODE_LENGTH);
		System.arraycopy(digest.getValue(), digest.getOffset(), origDigest.getValue(), 0,
		        MESSAGE_AUTHENTICATION_CODE_LENGTH);
