...
public abstract class PrivAES implements PrivacyProtocol {
	...
	public byte[] encrypt(byte[] unencryptedData, int offset, int length, byte[] encryptionKey, long engineBoots,
		...
		if (encryptionKey.length < keyBytes) {
			throw new IllegalArgumentException("Needed key length is " + keyBytes + ". Got only "
		...
		try {
			...
			SecretKeySpec key = new SecretKeySpec(encryptionKey, 0, keyBytes, "AES");
	...
	public byte[] decrypt(byte[] cryptedData, int offset, int length, byte[] decryptionKey, long engineBoots,
		...
		try {
			...
			Cipher alg = Cipher.getInstance("AES/CFB/NoPadding");
			SecretKeySpec key = new SecretKeySpec(decryptionKey, 0, keyBytes, "AES");
			IvParameterSpec ivSpec = new IvParameterSpec(initVect);
