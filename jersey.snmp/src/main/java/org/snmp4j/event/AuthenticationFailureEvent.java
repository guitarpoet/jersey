...
/**
 * The <code>AuthenticationFailureEvent</code> class describes the source and
 * type of an authentication failure as well as the message that caused the
 * error.
 * 
 * @author Frank Fock
 * @version 1.5
...
public class AuthenticationFailureEvent extends EventObject {
	 ...
	 * @param transport
	 *            the <code>TransportMapping</code> with which the message has
	 *            been received.
	 * @param error
	 *            the SNMP4J MP error status caused by the message (see
	 *            {@link SnmpConstants}).
	 * @param message
	 *            the message as received at the position where processing the
	 *            message has stopped.
	 */
	public AuthenticationFailureEvent(TransportListener source, Address sourceAddress, TransportMapping<?> transport,
	        int error, BERInputStream message) {
		super(source);
