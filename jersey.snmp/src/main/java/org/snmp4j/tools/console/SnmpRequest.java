...
public class SnmpRequest implements CommandResponder, PDUFactory {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-a")) {
				...
				if (!snapshotFile.canWrite() && snapshotFile.exists()) {
					throw new IllegalArgumentException("Snapshot file '" + snapshotFile + "' cannot be written");
				}
			} else if (args[i].equals("-Ods")) {
				operation = SNAPSHOT_DUMP;
				snapshotFile = new File(nextOption(args, i++));
				if (!snapshotFile.canRead()) {
					throw new IllegalArgumentException("Snapshot file '" + snapshotFile + "' cannot be read");
				}
	...
	protected static void printVersion() {
		System.out.println();
		System.out.println("SNMP4J Command Line Tool v" + VersionInfo.getVersion() + " Copyright " + (char) 0xA9
	...
	protected static void printUsage() {
		String[] usage = new String[] {
		        ...
		        "has the same effect as enumerating all OIDs from '1.3.6.1.2.1.2.2.1' to", "'1.3.6.1.2.1.2.2.10'.",
		        ...
		        "the following single characters enclosed by '{' and '}':", "  i                     Integer32",
		        "  u                     UnsingedInteger32, Gauge32", "  s                     OCTET STRING",
		        "  x                     OCTET STRING specified as hex string where",
		        "                        bytes separated by colons (':').",
		        "  d                     OCTET STRING specified as decimal string",
		        "                        where bytes are separated by dots ('.').", "  n                     Null",
		        "  o                     OBJECT IDENTIFIER", "  t                     TimeTicks",
		        "  a                     IpAddress",
		        ...
		        " SNMP4J -u aSecurityName -Ol 0.0.0.0/162", "", "Send an unauthenticated SNMPv3 notification (trap):",
			...
			if (snmpRequest.operation == SNAPSHOT_DUMP) {
				...
				try {
					if (snmpRequest.operation == LISTEN) {
						snmpRequest.listen();
					} else if ((snmpRequest.operation == TABLE) || (snmpRequest.operation == CVS_TABLE)
					        || (snmpRequest.operation == TIME_BASED_CVS_TABLE)) {
						snmpRequest.table();
					} else {
						PDU response = snmpRequest.send();
						if ((snmpRequest.getPduType() == PDU.TRAP) || (snmpRequest.getPduType() == PDU.REPORT)
							...
							System.out.println("Response received with requestID=" + response.getRequestID()
							        + ", errorIndex=" + response.getErrorIndex() + ", " + "errorStatus="
							        + response.getErrorStatusText() + "(" + response.getErrorStatus() + ")");
							printVariableBindings(response);
						} else {
							System.out.println("Received something strange: requestID=" + response.getRequestID()
							        + ", errorIndex=" + response.getErrorIndex() + ", " + "errorStatus="
							        + response.getErrorStatusText() + "(" + response.getErrorStatus() + ")");
							printVariableBindings(response);
						}
