...
/**
 * The <code>LogControl</code> tool can be used to control the log levels of
 * agents implementing the SNMP4J-LOG-MIB.
 * 
...
public class LogControl {
	...
	private static final String OPTIONS = "+a[s{=MD5}<(MD5|SHA)>] " + "+A[s] +b[i{=0}] "
	        + "-c[s{=public}] -bc[i{=0}] +u[s{securityName}] -t[l{timeout=5000}] -r[i{retries=0}] "
	        + "+l[o<\\n\\n[:\\n\\n]*>] " + "+e[o<\\n\\n[:\\n\\n]*>] " + "+E[o<\\n\\n[:\\n\\n]*>] " + "+n[s] "
	        + "+Y[s] +y[s<(DES|3DES|AES|AES128|AES192|AES256)>] " + "-v[s{version=3}<(1|2c|3)>] ";

	public static final String COMMAND_PARAMETER = "#command[s<(set|list)>] +following[s] ..";

	private static final String[][] COMMANDS = {
	        { "list", OPTIONS, "#command[s<list>] #address[s<(udp|tcp):.*[/[0-9]+]?>] +filter[s]" },
	...
	public void run() {
		...
		if ("help".equals(command)) {
			...
			try {
				...
				OctetString localEngineID = new OctetString(MPv3.createLocalEngineID(new OctetString("LogControl"
	...
	private void verifyLoggerModification(Snmp snmp, Target target, PDU pdu, OID levelOID, String logger,
	        OID loggerIndex, String newLevel, ResponseEvent response) throws IOException {
		pdu.clear();
		...
		if ((respPDU != null) && (respPDU.getErrorStatus() == PDU.noError) && (!respPDU.get(0).isException())
			...
			System.out.println("Set logger '" + logger + "' level to " + newLevel + ". Now levels are " + setLevel
			        + " (configured) and " + effectiveLevel + " (effective).");
		} else {
