...
/**
 * The <code>TLSTM</code> implements the Transport Layer Security Transport
 * Mapping (TLS-TM) as defined by RFC 5953 with the new IO API and
 * {@link javax.net.ssl.SSLEngine}.
 * <p>
 * It uses a single thread for processing incoming and outgoing messages. The
 * thread is started when the <code>listen</code> method is called, or when an
 * outgoing request is sent using the <code>sendMessage</code> method.
 * 
 * @author Frank Fock
 * @version 2.0
 * @since 2.0
...
public class TLSTM extends TcpTransportMapping {
	 ...
	 * bind to the given address. The <code>securityCallback</code> needs to be
	 * specified before {@link #listen()} is called.
	 * 
	...
	public TLSTM(TlsAddress address) throws IOException {
		...
		try {
			if (Class.forName("javax.net.ssl.X509ExtendedTrustManager") != null) {
				Class trustManagerFactoryClass = Class
				        .forName("org.snmp4j.transport.tls.TLSTMExtendedTrustManagerFactory");
	 ...
	 *            a security name callback to resolve X509 certificates to
	 *            tmSecurityNames.
	 * @param serverAddress
	 *            the TcpAddress instance that describes the server address to
	 *            listen on incoming connection requests.
	 * @throws java.io.IOException
	 ...
	 *            The CounterSupport instance to be used to count events created
	 *            by this TLSTM instance. To get a default instance, use
	 *            {@link CounterSupport#getInstance()}.
	 * @throws java.io.IOException
	 *             if the given address cannot be bound.
	 */
	public TLSTM(TlsTmSecurityCallback<X509Certificate> securityCallback, TlsAddress serverAddress,
	        CounterSupport counterSupport) throws IOException {
		super(serverAddress);
		super.maxInboundMessageSize = MAX_TLS_PAYLOAD_SIZE;
		this.serverEnabled = true;
	 ...
	 * 
	 * @param tlsProtocols
	 *            an array of TLS protocol (version) names supported by the
	 *            SunJSSE provider. The order in the array defines which
	 *            protocol is tried during handshake first.
	 * @since 2.0.3
	 */
	 ...
	 * this TLSTM. Setting this property to a value other than <code>null</code>
	 * filters out any certificates which are not in the chain of the given
	 * alias.
	 * 
	 * @param localCertificateAlias
	 *            a certificate alias which filters a single certification chain
	 *            from the <code>javax.net.ssl.keyStore</code> key store to be
	 *            used to authenticate this TLS transport mapping. If
	 *            <code>null</code> no filtering appears, which could lead to
	 *            more than a single chain available for authentication by the
	 *            peer, which would violate the TLSTM standard requirements.
	 */
	 ...
	 * <code>serverEnabled</code> member is <code>false</code> the server for
	 * incoming requests is not started. This starts the internal server thread
	 * that processes messages.
	 * 
	 * @throws java.net.SocketException
	 *             when the transport is already listening for incoming/outgoing
	 *             messages.
	 * @throws java.io.IOException
	 */
	public synchronized void listen() throws IOException {
		...
		server = SNMP4JSettings.getThreadFactory().createWorkerThread("TLSTM_" + getAddress(), serverThread, true);
	 ...
	 * @throws java.io.IOException
	 *             if the remote address cannot be closed due to an IO
	 *             exception.
	 * @since 1.7.1
	 */
	 ...
	 * 
	 * @param address
	 *            an <code>TcpAddress</code>. A <code>ClassCastException</code>
	 *            is thrown if <code>address</code> is not a
	 *            <code>TcpAddress</code> instance.
	 * @param message
	 ...
	 *            the idle timeout in milliseconds. A zero or negative value
	 *            will disable any timeout and connections opened by this
	 *            transport mapping will stay opened until they are explicitly
	 *            closed.
	 */
	 ...
	 * 
	 * @param serverEnabled
	 *            if <code>true</code> if the transport will listens for
	 *            incoming requests after {@link #listen()} has been called.
	 */
	...
	public void setMessageLengthDecoder(MessageLengthDecoder messageLengthDecoder) {
		/*
		 * if (messageLengthDecoder == null) { throw new NullPointerException();
	...
	public static OctetString getFingerprint(X509Certificate cert) {
		...
		try {
			...
			logger.error(
			        "No such digest algorithm exception while getting fingerprint from " + cert + ": " + e.getMessage(),
			        e);
		} catch (CertificateEncodingException e) {
			logger.error(
			        "Certificate encoding exception while getting fingerprint from " + cert + ": " + e.getMessage(), e);
		}
	 ...
	 * 
	 * @param serverSocket
	 *            the <code>ServerSocket</code> to apply additional non-default
	 *            options.
	 */
	...
	class SocketEntry {
		...
		public SocketEntry(TcpAddress address, Socket socket, boolean useClientMode,
			...
			// sslEngineConfigurator.configure(SSLContext.getDefault(),
			// useClientMode);
		...
		public String toString() {
			return "SocketEntry[peerAddress=" + peerAddress + ",socket=" + socket + ",lastUse=" + new Date(lastUse)
			        + ",inNetBuffer=" + inNetBuffer + ",inAppBuffer=" + inAppBuffer + ",outNetBuffer=" + outNetBuffer
			        + "]";
		}

		/*
		 * public boolean equals(Object o) { if (o instanceof SocketEntry) {
		 * SocketEntry other = (SocketEntry)o; return
		 * other.peerAddress.equals(peerAddress) && ((other.message == message)
		 * || ((message != null) && (message.equals(other.message)))); } return
		 * false; }
		 * 
		 * public int hashCode() { return peerAddress.hashCode(); }
		 */

		public void checkTransportStateReference() {
			if (tmStateReference == null) {
				tmStateReference = new TransportStateReference(TLSTM.this, peerAddress, new OctetString(),
				        SecurityLevel.authPriv, SecurityLevel.authPriv, true, sessionID);
				OctetString securityName = null;
				if (securityCallback != null) {
					try {
						securityName = securityCallback.getSecurityName((X509Certificate[]) sslEngine.getSession()
						        .getPeerCertificates());
		...
		public void closeSession() {
			...
			/*
			 * if (sslEngine.isOutboundDone()) { // try to receive close alert
			 * message SSLEngineResult result; try { int i=0; do { synchronized
			 * (this.inboundLock) { this.inNetBuffer.flip();
			 * this.inNetBuffer.limit(this.inNetBuffer.capacity());
			 * logger.debug("TLS inNetBuffer = "+this.inNetBuffer); result =
			 * this.sslEngine.unwrap(this.inNetBuffer, this.inAppBuffer); //
			 * adjustInNetBuffer(this, result); } } while ((result.getStatus()
			 * != SSLEngineResult.Status.CLOSED) && (i++ < 5) &&
			 * !sslEngine.isInboundDone() && (result.getHandshakeStatus() ==
			 * SSLEngineResult.HandshakeStatus.NEED_UNWRAP));
			 * sslEngine.closeInbound(); } catch (SSLException e) {
			 * logger.error("SSLException while closing inbound channel of " +
			 * this + ": " + e.getMessage(), e); } }
			 */
		}
	}
	...
	class SocketTimeout extends TimerTask {
		...
		public void run() {
			...
			if ((socketCleaner == null) || ((now - entry.getLastUse()) / 1000000 >= connectionTimeout)) {
				if (logger.isDebugEnabled()) {
					logger.debug("Socket has not been used for " + (now - entry.getLastUse())
				...
				long nextRun = System.currentTimeMillis() + (now - entry.getLastUse()) / 1000000 + connectionTimeout;
				if (logger.isDebugEnabled()) {
	...
	class ServerThread implements WorkerTask {
		...
		public ServerThread() throws IOException, NoSuchAlgorithmException {
			...
			if (serverEnabled) {
				...
				// the
				// ready list when accept operations occur, so allowing
				// multiplexed
				...
				while (!inQueue.isEmpty()) {
					...
					try {
						...
						synchronized (entry.inboundLock) {
							...
							/*
							 * if (entry.inNetBuffer.limit() <
							 * entry.inNetBuffer.capacity()) {
							 * entry.inNetBuffer.
							 * limit(entry.inNetBuffer.capacity());
							 * entry.addRegistration(selector,
							 * SelectionKey.OP_READ); continue; }
							 */
							...
							if (runDelegatedTasks(nextResult, entry)) {
								...
								case OK:
									...
									if (entry.inAppBuffer.limit() > 0) {
										dispatchMessage(entry.getPeerAddress(), entry.inAppBuffer,
										        entry.inAppBuffer.limit(), entry.sessionID, entry.tmStateReference);
									}
				...
				for (int i = 0; i < pending.size(); i++) {
					...
					try {
						...
						try {
							...
							TransportStateEvent e = new TransportStateEvent(TLSTM.this, entry.getPeerAddress(),
						...
						try {
							entry.getSocket().getChannel().close();
							TransportStateEvent e = new TransportStateEvent(TLSTM.this, entry.getPeerAddress(),
							        TransportStateEvent.STATE_CLOSED, iox);
							fireConnectionStateChanged(e);
						} catch (IOException ex) {
							logger.error(ex);
						}
		...
		public boolean runDelegatedTasks(SSLEngineResult result, SocketEntry entry) throws IOException {
			...
			case FINISHED:
				...
				entry.setHandshakeFinished(true);/*
												 * if (result.bytesProduced() >
												 * 0) { writeNetBuffer(entry,
												 * entry
												 * .getSocket().getChannel()); }
												 * /* if
												 * (entry.isAppOutPending()) {
												 * writeMessage(entry,
												 * entry.getSocket
												 * ().getChannel()); }
												 */
		...
		public void sendMessage(Address address, byte[] message, TransportStateReference tmStateReference)
			...
			if (entry != null) {
				if ((tmStateReference != null) && (tmStateReference.getSessionID() != null)
					...
					counterSupport
					        .fireIncrementCounter(new CounterEvent(this, SnmpConstants.snmpTlstmSessionNoSessions));
				...
				try {
					...
					if ((s == null) || (s.isClosed())) {
						...
						if (!sc.isConnectionPending()) {
							sc.connect(targetAddress);
							counterSupport.fireIncrementCounter(new CounterEvent(this,
							...
							if (matchingStateReferences(tmStateReference, entry.tmStateReference)) {
								...
								logger.error("TransportStateReferences refNew=" + tmStateReference + ",refOld="
								        + entry.tmStateReference + " do not match, message dropped");
					...
					logger.error(
					        "NoSuchAlgorithmException while sending message to " + address + ": " + e.getMessage(), e);
				}
							...
							while (it.hasNext()) {
								try {
									...
									if (sk.isAcceptable()) {
										...
										// The key indexes into the selector so
										// you
										// can retrieve the socket that's ready
										// for I/O
										...
										timeoutSocket(entry);
										TransportStateEvent e = new TransportStateEvent(TLSTM.this, incomingAddress,
										...
										incomingAddress = new TcpAddress(readChannel.socket().getInetAddress(),
									...
									if (readChannel != null) {
										...
										try {
											...
											// IO exception -> channel closed
											// remotely
											logger.warn(iox);
											iox.printStackTrace();
											sk.cancel();
											readChannel.close();
											TransportStateEvent e = new TransportStateEvent(TLSTM.this,
											        incomingAddress, TransportStateEvent.STATE_DISCONNECTED_REMOTELY,
											        iox);
											fireConnectionStateChanged(e);
										}
									}
									...
									logger.error("NoSuchAlgorithm while reading from server socket: " + e.getMessage(),
									        e);
								}
							}
						}
					} catch (NullPointerException npex) {
						// There seems to happen a NullPointerException within
						// the select()
			...
			try {
				...
				if (!sc.isConnected()) {
					if (sc.finishConnect()) {
						...
						// make sure connection is closed if not used for
						// timeout
						// micro seconds
						timeoutSocket(entry);
				...
				if (entry != null) {
					...
					TransportStateEvent e = new TransportStateEvent(TLSTM.this, addr,
					        TransportStateEvent.STATE_CONNECTED, null);
					fireConnectionStateChanged(e);
				}
			} catch (IOException iox) {
		...
		protected ByteBuffer createBufferCopy(ByteBuffer buffer) {
		...
		private void dispatchMessage(TcpAddress incomingAddress, ByteBuffer byteBuffer, long bytesRead,
		        Object sessionID, TransportStateReference tmStateReference) {
			byteBuffer.flip();
			if (logger.isDebugEnabled()) {
			...
			synchronized (entry.outboundLock) {
				if (entry.outAppBuffer == null) {
					...
					if (message != null) {
						...
						if (logger.isDebugEnabled()) {
							logger.debug("Sending message with length " + message.length + " to "
							        + entry.getPeerAddress() + ": " + new OctetString(message).toHexString());
						}
					} else {
						entry.removeRegistration(selector, SelectionKey.OP_WRITE);
						// Make sure that we did not clear a selection key that
						// was concurrently
						// added:
				...
				if (result.getStatus() == SSLEngineResult.Status.OK) {
					...
					/*
					 * if (entry.isAppOutPending()) { writeMessage(entry,
					 * entry.getSocket().getChannel()); }
					 */
				}
	...
	private boolean matchingStateReferences(TransportStateReference tmStateReferenceNew,
		...
		if ((tmStateReferenceExisting == null) || (tmStateReferenceNew == null)) {
			logger.error("Failed to compare TransportStateReferences refNew=" + tmStateReferenceNew + ",refOld="
			        + tmStateReferenceExisting);
			return false;
		}
		if ((tmStateReferenceNew.getSecurityName() == null) || (tmStateReferenceExisting.getSecurityName() == null)) {
			logger.error("Could not match TransportStateReferences refNew=" + tmStateReferenceNew + ",refOld="
			        + tmStateReferenceExisting);
			return false;
	...
	static interface SSLEngineConfigurator {
		 ...
		 * 
		 * @param useClientMode
		 *            <code>true</code> if the connection is established in
		 *            client mode.
		 * @param transportStateReference
		 *            the transportStateReference with additional security
		 *            information for the SSL connection to establish.
		...
		public SSLContext getSSLContext(boolean useClientMode, TransportStateReference transportStateReference) {
			try {
				...
				try {
					...
					sslContext.init(kmf.getKeyManagers(), new TrustManager[] { trustManagerFactory.create(
					        (X509TrustManager) trustManagers[0], useClientMode, transportStateReference) }, null);
					return sslContext;
				} catch (KeyStoreException e) {
					logger.error("Failed to initialize SSLContext because of a KeyStoreException: " + e.getMessage(), e);
				} catch (KeyManagementException e) {
					logger.error(
					        "Failed to initialize SSLContext because of a KeyManagementException: " + e.getMessage(), e);
				} catch (UnrecoverableKeyException e) {
					logger.error(
					        "Failed to initialize SSLContext because of an UnrecoverableKeyException: "
					                + e.getMessage(), e);
				} catch (CertificateException e) {
					logger.error(
					        "Failed to initialize SSLContext because of a CertificateException: " + e.getMessage(), e);
				} catch (FileNotFoundException e) {
					logger.error(
					        "Failed to initialize SSLContext because of a FileNotFoundException: " + e.getMessage(), e);
				} catch (IOException e) {
					logger.error("Failed to initialize SSLContext because of an IOException: " + e.getMessage(), e);
				}
			} catch (NoSuchAlgorithmException e) {
				logger.error(
				        "Failed to initialize SSLContext because of an NoSuchAlgorithmException: " + e.getMessage(), e);
			}
			return null;
		}
		...
		private void filterCertificates(KeyStore ks, TransportStateReference transportStateReference) {
			...
			if (localCertAlias != null) {
				try {
					...
					if (chain == null) {
						logger.warn("Local certificate with alias '" + localCertAlias
						        + "' not found. Known aliases are: " + Collections.list(ks.aliases()));
					} else {
					...
					logger.error("Failed to get certificate chain for alias " + localCertAlias + ": " + e.getMessage(),
					        e);
				}
			}
		}
			...
			if (useClientMode && (callback != null)) {
				if (!callback.isServerCertificateAccepted(x509Certificates)) {
					...
					throw new CertificateException(
					        "Server's certificate is not trusted by this application (although it was trusted by the JRE): "
					                + Arrays.asList(x509Certificates));
				}
			}
		}
			...
			if ((fingerprint != null) && (fingerprint.length() > 0)) {
				for (X509Certificate cert : x509Certificates) {
					...
					if (certFingerprint == null) {
						logger.error("Failed to determine fingerprint for certificate " + cert + " and algorithm "
						        + cert.getSigAlgName());
