...
/**
 * The <code>DummyTransport</code> is a test TransportMapping for Command
 * Generators which actually does not sent messages over the network. Instead it
 * provides the message transparently as incoming message over the
 ...
 * if the <code>listenAddress</code> is left <code>null</code>.
 * 
 * @author Frank Fock
 * @since 2.0
 */
public class DummyTransport<A extends IpAddress> extends AbstractTransportMapping<A> {
	...
	private class QueueProcessor implements WorkerTask {
		...
		public void run() {
			while (!stop) {
				...
				if (nextMessage != null) {
					TransportStateReference stateReference = new TransportStateReference(DummyTransport.this,
					        listenAddress, null, SecurityLevel.undefined, SecurityLevel.undefined, false, sessionID);
	...
	public String toString() {
		return "DummyTransport{" + "requests=" + requests + ", responses=" + responses + ", listening=" + listening
		        + ", listenAddress=" + listenAddress + ", receiverAddress=" + receiverAddress + ", listenThread="
		        + listenThread + ", sessionID=" + sessionID + '}';
	}
}
