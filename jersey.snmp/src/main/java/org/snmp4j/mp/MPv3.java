...
public class MPv3 implements MessageProcessingModel {
	...
	private static final int MAX_HEADER_LENGTH = MAX_HEADER_PAYLOAD_LENGTH
	 ...
	 *            a {@link PDUFactory}. If <code>null</code> the default factory
	 *            will be used which creates {@link ScopedPDU} instances.
	 * @since 1.9.1
	 */
	public MPv3(byte[] localEngineID, PDUFactory incomingPDUFactory) {
		this(localEngineID, incomingPDUFactory, SecurityProtocols.getInstance(), SecurityModels.getInstance(),
	 ...
	 */
	public MPv3(USM usm) {
		this(usm.getLocalEngineID().getValue(), null, SecurityProtocols.getInstance(), SecurityModels
		        .getCollection(new SecurityModel[] { usm }), CounterSupport.getInstance());
	}
	 ...
	 *            will be used which creates {@link ScopedPDU} instances.
	 * @param secProtocols
	 *            the SecurityProtocols instance to use when looking up a
	 *            security protocol. To get a default instance, use
	 *            {@link SecurityProtocols#getInstance()}.
	 * @param secModels
	 *            the SecurityModels instance to use when looking up a security
	 *            model. If you use more than one USM instance, you need to
	 *            create a SecurityProtocols instance (container) for each such
	 *            USM instance (and MPv3 combination). To get a default
	 *            instance, use {@link SecurityProtocols#getInstance()}.
	 * @param counterSupport
	 *            The CounterSupport instance to be used to count events created
	 *            by this MPv3 instance. To get a default instance, use
	 *            {@link CounterSupport#getInstance()}.
	 * @since 1.10
	 */
	public MPv3(byte[] localEngineID, PDUFactory incomingPDUFactory, SecurityProtocols secProtocols,
	        SecurityModels secModels, CounterSupport counterSupport) {
	...
	public void setLocalEngineID(byte[] engineID) {
		if ((engineID == null) || (engineID.length < MINLEN_ENGINE_ID) || (engineID.length > MAXLEN_ENGINE_ID)) {
	 ...
	 * @return <code>true</code> if the engine ID has been added,
	 *         <code>false</code> otherwise (if the supplied
	 *         <code>engineID</code> equals the local one).
	 */
	public boolean addEngineID(Address address, OctetString engineID) {
		if (!Arrays.equals(this.localEngineID, engineID.getValue())) {
			...
			if (snmpEngineListeners != null) {
				fireEngineChanged(new SnmpEngineEvent(this, SnmpEngineEvent.ADDED_ENGINE_ID, engineID, address));
			}
			return true;
		}
		return false;
	 ...
	 * 
	 * @param address
	 *            the <code>Address</code> of the remote SNMP engine for whose
	 *            engine ID is to be removed.
	 * @return the removed engine ID of the remote SNMP engine or
	 *         <code>null</code> if there is no entry for <code>address</code>
	...
	/**
	 * The <code>CacheEntry</code> class holds state reference information for
	 * the MPv3 message processing model for a single message.
	 * 
	 * @author Frank Fock
	 * @version 1.0
	 */
	protected static class CacheEntry extends StateReference {
		private static final long serialVersionUID = 6874490240012513584L;
		int msgID;
		long transactionID;
		byte[] secEngineID;
		...
		public CacheEntry(int msgID, long reqID, byte[] secEngineID, SecurityModel secModel, byte[] secName,
		        int secLevel, byte[] contextEngineID, byte[] contextName, SecurityStateReference secStateReference,
		        int errorCode) {
			this.msgID = msgID;
	...
	protected static class Cache {
		...
		/**
		 * Adds a <code>StateReference</code> to the cache. The
		 ...
		 *         already exists and {@link SnmpConstants#SNMP_MP_OK} on
		 *         success.
		 */
		public synchronized int addEntry(StateReference entry) {
			...
			// because we are using a weak has map for the cache, we need to
			// null out
		 ...
		 * @param pduHandle
		 *            a pduHandle.
		 * @return <code>true</code> if an entry has been deleted,
		 ...
		 *         or <code>null</code> if such an entry cannot be found. If a
		 *         cache entry is returned, the same is removed from the cache.
		 */
	...
	protected static class HeaderData implements BERSerializable {
		...
		public void decodeBER(BERInputStream message) throws IOException {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("SNMPv3 header decoded: msgId=" + msgID + ", msgMaxSize=" + msgMaxSize + ", msgFlags="
				        + msgFlags.toHexString() + ", secModel=" + securityModel);
			}
	...
	public int prepareOutgoingMessage(Address transportAddress, int maxMessageSize, int messageProcessingModel,
	        int securityModel, byte[] securityName, int securityLevel, PDU pdu, boolean expectResponse,
	        PduHandle sendPduHandle, Address destTransportAddress, BEROutputStream outgoingMessage,
		...
		int status = secModel.generateRequestMessage(messageProcessingModel, globalDataBuffer.array(), maxMessageSize,
		        securityModel, secEngineID, securityName, securityLevel, scopedPDUInput, securityParameters,
		        outgoingMessage, tmStateReference);
		if (status == SnmpConstants.SNMPv3_USM_OK) {
			if (expectResponse) {
				cache.addEntry(new StateReference(msgID, flags, maxMessageSize, sendPduHandle, transportAddress, null,
				        secEngineID, secModel, securityName, securityLevel, scopedPDU.getContextEngineID().getValue(),
				        scopedPDU.getContextName().getValue(), null, status));
			}
		}
		return status;
	}
	...
	public int prepareResponseMessage(int messageProcessingModel, int maxMessageSize, int securityModel,
	        byte[] securityName, int securityLevel, PDU pdu, int maxSizeResponseScopedPDU,
	        StateReference stateReference, StatusInformation statusInformation, BEROutputStream outgoingMessage)
	        throws IOException {
		...
		int status = secModel.generateResponseMessage(getID(), globalDataBuffer.array(), maxMessageSize, securityModel,
		        securityEngineID.getValue(), securityName, securityLevel, scopedPDUInput,
		        cacheEntry.getSecurityStateReference(), securityParameters, outgoingMessage);
		return status;
	}
	 ...
	 *            Send the message on behalf the supplied MessageDispatcher
	 *            instance.
	 * @param pdu
	 *            ScopedPDU If <code>null</code>, then contextEngineID,
	 *            contextName, and requestID of the report generated will be
	 *            zero length and zero respective. Otherwise these values are
	 *            extracted from the PDU.
	 * @param securityLevel
	 ...
	 *            the maximum size of of the report message (will be most likely
	 *            ignored because a report should always fit in 484 bytes).
	 ...
	 *         successfully.
	 */
	public int sendReport(MessageDispatcher messageDispatcher, ScopedPDU pdu, int securityLevel, int securityModel,
	        OctetString securityName, int maxSizeResponseScopedPDU, StateReference stateReference,
		...
		try {
			int status = messageDispatcher.returnResponsePdu(getID(), securityModel, securityName.getValue(),
			        securityLevel, reportPDU, maxSizeResponseScopedPDU, stateReference, statusInformation);
	...
	public int prepareDataElements(MessageDispatcher messageDispatcher, Address transportAddress,
	        BERInputStream wholeMsg, TransportStateReference tmStateReference, Integer32 messageProcessingModel,
	        Integer32 securityModel, OctetString securityName, Integer32 securityLevel, MutablePDU pdu,
	        PduHandle sendPduHandle, Integer32 maxSizeResponseScopedPDU, StatusInformation statusInformation,
		...
		try {
			...
			int status = secModel.processIncomingMsg(snmpVersion.getValue(),
			        header.getMsgMaxSize() - MAX_HEADER_LENGTH, secParameters, secModel, securityLevel.getValue(),
			        wholeMsg, tmStateReference,
			        // output parameters
			        securityEngineID, securityName, scopedPDU, maxSizeResponseScopedPDU, secStateReference,
			        statusInformation);
			wholeMsg.close();
			if (status == SnmpConstants.SNMPv3_USM_OK) {
				try {
				...
				if (((scopedPdu.getContextEngineID() == null) || (scopedPdu.getContextEngineID().length() == 0))
			...
			if (status != SnmpConstants.SNMPv3_USM_OK) {
				if ((reportableFlag) && (statusInformation.getErrorIndication() != null)) {
					...
					StateReference cacheEntry = new StateReference(header.getMsgID(), header.getMsgFlags(),
					        maxSizeResponseScopedPDU.getValue(), sendPduHandle, transportAddress, null,
					        securityEngineID.getValue(), secModel, securityName.getValue(), securityLevel.getValue(),
					...
					int reportStatus = sendReport(messageDispatcher, scopedPdu, statusInformation.getSecurityLevel()
					        .getValue(), secModel.getID(), securityName, maxSizeResponseScopedPDU.getValue(),
					        stateReference, statusInformation.getErrorIndication());
			...
			if ((scopedPdu.getType() == PDU.RESPONSE) || (scopedPdu.getType() == PDU.REPORT)) {
				...
				if (cacheEntry != null) {
					if (logger.isDebugEnabled()) {
						logger.debug("RFC3412 ��7.2.10 - Received PDU (msgID=" + header.getMsgID()
						        + ") is a response or " + "an internal class message. PduHandle.transactionID = "
					...
					if (scopedPdu.getType() == PDU.REPORT) {
						...
						if (((cacheEntry.getSecurityEngineID().length != 0) && (!securityEngineID
						        .equalsValue(cacheEntry.getSecurityEngineID())))
						        || (secModel.getID() != cacheEntry.getSecurityModel().getID())
						        || ((!securityName.equalsValue(cacheEntry.getSecurityName()) && (securityName.length() != 0)))) {
							if (logger.isDebugEnabled()) {
								logger.debug("RFC 3412 ��7.2.11 - Received report message does not match sent message. Cache entry is: "
								        + cacheEntry
								        + ", received secName="
								        + securityName
								        + ",secModel="
						...
						if (!addEngineID(cacheEntry.getAddress(), securityEngineID)) {
							if (logger.isWarnEnabled()) {
								logger.warn("Engine ID '" + securityEngineID
								        + "' could not be added to engine ID cache for " + "target address '"
								        + cacheEntry.getAddress() + "' because engine ID matches local engine ID");
							}
						}
						// cache.deleteEntry(cacheEntry.getPduHandle());
						mutableStateReference.setStateReference(null);
					...
					if (scopedPdu.getType() == PDU.RESPONSE) {
						if (((!securityEngineID.equalsValue(cacheEntry.getSecurityEngineID())) && (cacheEntry
						        .getSecurityEngineID().length != 0))
						        || (secModel.getID() != cacheEntry.getSecurityModel().getID())
						        || (!securityName.equalsValue(cacheEntry.getSecurityName()))
						        || (securityLevel.getValue() != cacheEntry.getSecurityLevel())
						        || ((!scopedPdu.getContextEngineID().equalsValue(cacheEntry.getContextEngineID())) && (cacheEntry
						                .getContextEngineID().length != 0))
						        || ((!scopedPdu.getContextName().equalsValue(cacheEntry.getContextName()) && (cacheEntry
						                .getContextName().length != 0)))) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("RFC3412 ��7.2.10 - Received PDU (msgID=" + header.getMsgID()
						        + ") is a response or " + "internal class message, but cached "
				...
				logger.debug("RFC3412 ��7.2.10 - Received PDU is NOT a response or "
				        + "internal class message -> unchanged PduHandle = " + sendPduHandle);
			}
			...
			case PDU.SET: {
				if (securityEngineID.length() == 0) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("RFC 3412 ��7.2.13.a - Security engine ID " + securityEngineID.toHexString()
						        + " does not match local engine ID " + new OctetString(localEngineID).toHexString());
					}
					mutableStateReference.setStateReference(null);
	 ...
	 *            an enterprise ID as registered by the IANA (see
	 *            http://www.iana.org).
	 */
	 ...
	 * @since 1.9.1
	 * @deprecated Use
	 *             {@link org.snmp4j.util.DefaultPDUFactory#createPDU(MessageProcessingModel, int)}
	 *             instead.
	 */
	public PDU createPDU(Target target) {
