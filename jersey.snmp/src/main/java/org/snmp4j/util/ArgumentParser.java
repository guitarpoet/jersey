 ...
 * where
 * <ul>
 * <li>'-' indicates a mandatory option ('+' would indicate an optional option)</li>
 ...
 * values</li> </li>&lt;default&gt; is a default value. If a default value is
 * given, then a mandatory option is in fact optional</li>
 * </ul>
 * </p>
 * <p>
...
public class ArgumentParser {
	 ...
	 *            the parameter format pattern to parse (see
	 *            {@link ArgumentParser}).
	 */
	 ...
	 *         have been parsed, where each map entry has the option/parameter
	 *         name as key and the value as value.
	 * @throws ParseException
	 *             if the command line does not match the patterns for options
	 *             and parameters.
	 */
				...
				for (int i = 0; i < of.getParameters().length; i++) {
					if (of.getParameters()[i].getDefaultValue() != null) {
						defaults.add(parseParameterValue(of.getParameters()[i],
						        of.getParameters()[i].getDefaultValue(), of, i));
					}
				}
				if (defaults.size() == 0) {
	...
	protected List parseValues(String[] args, int offset, ArgumentFormat format) throws ParseException {
		...
		for (int i = 0; (i + offset < args.length) && (i < numParams); i++) {
			try {
				values.add(parseParameterValue(format.getParameters()[i], args[i + offset], format, i + offset));
	...
	protected Object parseParameterValue(ArgumentParameter type, String value, ArgumentFormat format, int pos)
	        throws org.snmp4j.util.ArgumentParser.ArgumentParseException {
		...
		if (type.pattern != null) {
			...
			if (!m.matches()) {
				throw new ArgumentParseException("Value '" + value + "' for "
				        + (format.isParameter() ? "parameter " : "option ") + format.getOption()
				        + ((format.getParameters().length > 1) ? " part " + type.getName() : "")
				        + " does not match pattern '" + type.pattern.pattern() + "'", pos, value, format, type);
			}
		}
		switch (type.getType()) {
		case TYPE_INTEGER:
	...
	public static class ArgumentFormat {
		...
		public String toString() {
			return "ArgumentFormat[option=" + option + ",parameter=" + parameter + ",vararg=" + vararg
	...
	public static class ArgumentParameter {
		...
		public String toString() {
			return "ArgumentParameter[name=" + name + ",type=" + type + ",patttern="
			        + ((pattern == null) ? null : pattern.pattern()) + ",defaultValue=" + defaultValue + "]";
		}
	}

	public static class ArgumentParseException extends ParseException {

		private static final long serialVersionUID = -7122382191465474333L;
		private ArgumentParameter parameterFormatDetail;
		private ArgumentFormat parameterFormat;
		private String value;

		public ArgumentParseException(int position, String value, ArgumentFormat parameterFormat,
		        ArgumentParameter parameterFormatDetail) {
			super((value != null) ? "Invalid value '" + value + "' at position " + position : "Mandatory parameter "
			        + parameterFormat.getOption() + "(" + parameterFormatDetail.getName() + ") not specified", position);
			this.parameterFormat = parameterFormat;
			this.parameterFormatDetail = parameterFormatDetail;
			this.value = value;
		}

		public ArgumentParseException(String message, int position, String value, ArgumentFormat parameterFormat,
		        ArgumentParameter parameterFormatDetail) {
	...
	/**
	 * Gets the <code>n</code>-th option value of a list of values - if
	 * available.
	 * 
	 * @param args
	 ...
	 * @param index
	 *            the index (zero based) of the option/parameter value to
	 *            return.
	 * @return the <code>n</code>-th (zero based) option value in
	 *         <code>args.get(name)</code> if it exists, <code>null</code>
	 *         otherwise.
	 * @since 1.10
	 */
	 ...
	 * system properties <code>org.snmp4j.OptionFormat</code> and
	 ...
	 * 
	 * @param args
	 *            the command line arguments to match with the specified format
	 *            patterns.
	 */
	public static void main(String[] args) {
		ArgumentParser argumentparser = new ArgumentParser(System.getProperty("org.snmp4j.OptionFormat",
		        "-o1[i{parameter1}] -o2[s,l]"), System.getProperty("org.snmp4j.ParameterFormat",
		        "-param1[i] -param2[s<(udp|tcp):.*[/[0-9]+]?>{=udp:127.0.0.1/161}] " + "+optParam1[l{=-100}] .."));
	 ...
	 * @param optionFormat
	 *            the option format which is common to all commands (or their
	 *            super set)
	 * @param commandSets
	 *            the possible command sets, where each set is identified by its
	 *            command string in the first element of the command set.
	 * @return the command set matching the command in the argument list.
	 * @throws ParseException
	 *             if the command found in <code>args</code> cannot be found in
	 *             the <code>commandSets</code>, or <code>null</code> if
	 *             <code>args</code> does not contain any command.
	 * @since 1.10
	 */
	public static String[] selectCommand(String[] args, String optionFormat, String[][] commandSets)
	        throws ParseException {
