...
public abstract class AbstractTarget implements Target {
	...
	private static final long serialVersionUID = -5949357102405769171L;
	...
	public void setMaxSizeRequestPDU(int maxSizeRequestPDU) {
		if (maxSizeRequestPDU < SnmpConstants.MIN_PDU_LENGTH) {
			throw new IllegalArgumentException("The minimum PDU length is: " + SnmpConstants.MIN_PDU_LENGTH);
		}
	 ...
	 * will be chosen by the supplied <code>address</code> of the target. If an
	 * entity supports more than one {@link TransportMapping} for an
	 * {@link Address} class, the the results are not defined. This situation
	 * can be controlled by setting this preferredTransports list.
	 * 
	 * @since 2.0
	 */
	...
	protected String toStringAbstractTarget() {
		return "address=" + getAddress() + ",version=" + version + ",timeout=" + timeout + ",retries=" + retries
		        + ",securityLevel=" + securityLevel + ",securityModel=" + securityModel + ",securityName="
		        + securityName + ",preferredTransports=" + preferredTransports;
	}
