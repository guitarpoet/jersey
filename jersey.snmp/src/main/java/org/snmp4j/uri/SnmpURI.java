...
public class SnmpURI {
	...
	public SnmpUriResponse sendByBinding(URI url, List<VariableBinding> values, int pduType)
			...
			if (responseEvent != null) {
				...
				if (responsePDU != null) {
					if (responsePDU.getErrorStatus() != PDU.noError) {
						...
						response = new SnmpUriResponse(Collections.<VariableBinding[]> singletonList(responsePDU
						        .getVariableBindings().toArray(new VariableBinding[responsePDU.size()])));
					}
				} else {
		...
		private SnmpUriResponse createResponse(TreeEvent event) {
			...
			if (event.getStatus() == TreeEvent.STATUS_OK) {
				...
				response = new SnmpUriResponse(SnmpUriResponse.Type.SECURITY_ERROR,
				        (event.getReportPDU().size() > 0 ? event.getReportPDU().get(0).toString()
				                : "<empty report PDU>"));
