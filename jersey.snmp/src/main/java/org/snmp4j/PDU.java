 ...
 * version supported by the BER decoding and encoding methods of this class is
 * v2.
 * <p>
...
public class PDU implements BERSerializable, Serializable {
	 ...
	 */
	public static final int wrongEncoding = SnmpConstants.SNMP_ERROR_WRONG_ENCODING;
	 ...
	 */
	public static final int inconsistentValue = SnmpConstants.SNMP_ERROR_INCONSISTENT_VALUE;

	/**
	 * The resource needed to assign a variable binding's value is presently
	 * unavailable, see error index.
	 */
	public static final int resourceUnavailable = SnmpConstants.SNMP_ERROR_RESOURCE_UNAVAILABLE;
	 ...
	 */
	public static final int authorizationError = SnmpConstants.SNMP_ERROR_AUTHORIZATION_ERROR;
	 ...
	 * see error index.
	 */
	public static final int inconsistentName = SnmpConstants.SNMP_ERROR_INCONSISTENT_NAME;
	 ...
	 * 
	 * @param vbs
	 *            an array of <code>VariableBinding</code> instances. The
	 *            instances in the array will be appended to the current list of
	 *            variable bindings in the PDU.
	 */
	 ...
	 * 
	 * @param vbs
	 *            an array of <code>VariableBinding</code> instances. For each
	 *            instance in the supplied array, a new VariableBinding created
	 *            by <code>new VariableBinding(OID)</code> will be appended to
	 *            the current list of variable bindings in the PDU.
	 * @since 1.8
	 */
	 ...
	 * 
	 * @param index
	 *            a zero based positive integer (
	 *            <code>0 <= index < {@link #size()}</code>)
	 * @return a VariableBinding instance. If <code>index</code> is out of
	 ...
	 *         prefix matches <code>oid</code>. If no such element could be
	 ...
	 * @param prefix
	 *            the search {@link OID}.
	 * @return a List of all {@link VariableBinding}s whose prefix matches
	 *         <code>oid</code>. If no such element could be found, an empty
	 *         List is returned.
	 */
	 ...
	 * @param index
	 *            a zero based positive integer (
	 *            <code>0 <= index < {@link #size()}</code>) If
	 *            <code>index</code> is out of bounds an exception is thrown.
	 * @param vb
	 ...
	 * 
	 * @param vbs
	 *            a list of {@link VariableBinding} instances which must not be
	 *            null.
	 * @since 2.1
	 */
	 ...
	 *         {@link SnmpConstants#SNMP_ERROR_MESSAGES} array for a valid error
	 *         status. "Unknown error: <errorStatusNumber>" is returned for any
	 *         other value.
	 */
	 ...
	 * 
	 * @param errorIndex
	 *            an integer value >= 0 where 1 denotes the first variable
	 *            binding.
	 */
	...
	public boolean isConfirmedPdu() {
		return ((type != PDU.REPORT) && (type != PDU.RESPONSE) && (type != PDU.TRAP) && (type != PDU.V1TRAP));
	}
	...
	public void decodeBER(BERInputStream inputStream) throws IOException {
		...
		if (inputStream.getPosition() - startPos != vbLength) {
			throw new IOException("Length of VB sequence (" + vbLength + ") does not match real length: "
	 ...
	 * for the <code>PDU</code> when sent.
	 * 
	 * @param requestID
	 *            a unique request ID.
	 */
	 ...
	 * 
	 * @param type
	 *            the string representation of a PDU type:
	 *            <code>GET, GETNEXT, GETBULK,
	 *    SET, INFORM, RESPONSE, REPORT, TRAP, V1TRAP)</code>.
	 * @return the corresponding PDU type constant, or
	 *         <code>Integer.MIN_VALUE</code> of the supplied type is unknown.
	 */
	...
	public boolean equals(Object obj) {
		if (obj instanceof PDU) {
			PDU o = (PDU) obj;
			return (type == o.type) && (AbstractVariable.equal(requestID, o.requestID))
